...
shinyServer(function(input, output,session){
  ...
  output$plot <- renderPlot({
    sdata<-score()
    d1000<-read.table("F:/Software Application/Lung_cancer/data/1000Genome_lung_cancer.txt",head=T,sep="\t",stringsAsFactors = F)
...
score.score()
  ...
  output$range <- renderText({
    calc.range <- reactive({
      ...
      selectdata<-sdata[1,c("Sum(OR)")]
      if(!is.null(selectdata)){
        if(selectdata==0){print("VCF文件未包含肺癌易感位点")}else{
        d1000<-read.table("F:/Software Application/Lung_cancer/data/1000Genome_lung_cancer.txt",header=TRUE,sep="\t",stringsAsFactors = F)
...
version3<-function(tabdata){
  version3database<-read.table("F:/Software Application/Lung_cancer/data/lung_cancer.txt",head=T,sep="\t",stringsAsFactors = F)
  cg<-c("C","G")
  at<-c("A","T")
...
vcf2tab <- function(vcf){
  ...
  GT1 <- substr(info,1,3)
  GT2 <- ""
  database4<-read.table("F:/Software Application/Lung_cancer/data/lung_cancer.txt",head=T,sep="\t",stringsAsFactors = F)
  DP <- ""
  AD <- ""
  PL <- ""
  dbSNP <- ""
